...
//		O = toolkit.getImage(Tetris.class.getResource("O.png"));
	...
	public void action(){
		...
		startAction();//开始游戏
		repaint();//重新绘制
		KeyAdapter l = new KeyAdapter() {//接收键盘输入，判断反应
	...
	private void paintNextOne(Graphics g) {
		...
		for(int i=0; i<cells.length; i++){
			Cell c = cells[i];
			int x = (c.getCol()+10) * CELL_SIZE-1;//右移十个单位
			int y = (c.getRow()+1) * CELL_SIZE-1;//目测是坐标
	...
	//在 Tetris 类 中添加 方法 paintWall
		...
		private static final int[] SCORE_TABLE={0,1,10,30,200};//计分规则
		//   
